...
protected:
	AttackableGameObject(const std::wstring &name, unsigned int maxHitPoints, unsigned int currentHitPoints);

public:
	virtual ~AttackableGameObject() {}

	virtual bool ReceiveDamage(int damage) = 0;
	virtual int GetLocationX() const = 0;
	virtual int GetLocationY() const = 0;
